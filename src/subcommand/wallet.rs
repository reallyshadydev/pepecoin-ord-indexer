...
pub(crate) fn initialize_wallet(options: &Options, seed: [u8; 64]) -> Result {
  ...
  if let Err(err) = client.create_wallet(&options.wallet, None, Some(true), None, None) {
    let message = err.to_string();
    if message.contains("Method not found") || message.contains("-32601") {
      bail!(
        "Your Pepecoin node does not implement `createwallet` ({message}).\n\n\
         `getnewaddress` is not a substitute: it only uses the node's legacy wallet. \
         This `ord` build expects Core-style descriptor multiwallet (`createwallet`, `importdescriptors`, Taproot `tr(...)`), which Pepecoin Core does not provide.\n\n\
         For Pepecoin ordinals wallet + inscribe + send, use **`ordpep`** from https://github.com/mvdnbrk/ord-pepecoin instead of this binary's `wallet` commands."
      );
    }
    return Err(err.into());
  }
