...
impl Server {
  ...
  async fn get_utxos_by_address(
    ...
    let mut tx_cache: HashMap<Txid, bitcoin::Transaction> = HashMap::new();
    // Plain (non-inscription) UTXOs in deterministic order; used for pagination.
    ...
    for outpoint in outpoints {
      ...
      let tx = match tx_cache.entry(txid) {
      ...
      let in_page =
        show_all || (slot >= start_index && slot <= start_index + items_per_page - 1);
      slot += 1;
  ...
  async fn pepemap_by_address(
    ...
    let owner_key = script_key.to_string();

    // Wallet inventory: every inscription at this address whose body is a valid pepemap claim.
    // One row per claimed block height (`number`); if the wallet somehow holds two inscriptions
    // claiming the same height, keep the earlier global inscription (lower inscription_number).
    let mut by_number: std::collections::BTreeMap<u32, PepemapEntry> =
      std::collections::BTreeMap::new();

    let outpoints = index
      .get_account_outputs(owner_key.clone())
      .map_err(|err| ServerError::BadRequest(err.to_string()))?;
    for outpoint in outpoints {
      ...
      for inscription_id in inscription_ids {
        let number = if let Some(n) = index
          .pepemap_number_for_inscription_id(inscription_id)
          .map_err(|err| ServerError::BadRequest(err.to_string()))?
        {
          n
        } else {
          let Some(inscription) = index
            .get_inscription_by_id(inscription_id)
            .map_err(|err| ServerError::BadRequest(err.to_string()))?
          else {
            continue;
          };
          let Some(n) = crate::pepemap::parse_number(&inscription) else {
            continue;
          };
          n
        };

        let inscription_entry = index
          .get_inscription_entry(inscription_id)
          .map_err(|err| ServerError::BadRequest(err.to_string()))?;
        let (block_height, block_time) = inscription_entry
          .as_ref()
          .map(|entry| (entry.height, entry.timestamp))
          .unwrap_or_default();
        let new_ord = inscription_entry
          .map(|e| e.inscription_number)
          .unwrap_or(u64::MAX);
        ...
        let replace = match by_number.get(&number) {
          None => true,
          Some(existing) => {
            let old_ord = index
              .get_inscription_entry(existing.inscription_id)
              .map_err(|err| ServerError::BadRequest(err.to_string()))?
              .map(|e| e.inscription_number)
              .unwrap_or(u64::MAX);
            new_ord < old_ord
        ...
        if replace {
          by_number.insert(number, entry);
        }
      }
    }

    let items: Vec<PepemapEntry> = by_number.into_values().collect();
