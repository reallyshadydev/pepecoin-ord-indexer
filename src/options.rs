...
impl Options {
  ...
  pub(crate) fn dogecoin_rpc_client(&self) -> Result<Client> {
    ...
    // Pepecoin returns `softforks` as a JSON array (legacy Core shape). The typed
    // `get_blockchain_info` struct expects a map and fails with "expected a map".
    let info: serde_json::Value = client.call("getblockchaininfo", &[])?;
    let chain_str = info["chain"]
      .as_str()
      .ok_or_else(|| anyhow!("getblockchaininfo: missing or non-string `chain`"))?;
  ...
  pub(crate) fn dogecoin_rpc_client_for_wallet_command(&self, create: bool) -> Result<Client> {
    ...
    // Pepecoin Core reports Bitcoin-style `version` (e.g. 1.1.0 → 1010000). The upstream
    // Dogecoin gate (1140600) rejects valid Pepecoin RPC nodes.
    const MIN_VERSION: usize = 1_010_000;

    let dogecoin_version = client.version()?;
    if dogecoin_version < MIN_VERSION {
      bail!(
        "Pepecoin-compatible RPC {} or newer required (getnetworkinfo.version), current is {}",
