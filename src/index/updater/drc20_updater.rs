...
use crate::drc20::{
  max_script_tick_key, min_script_tick_key, script_tick_key, Balance, BlockContext, DRC20Error,
  Deploy, DeployEvent, Event, InscripbeTransferEvent, Message, Mint, MintEvent, Num, Tick,
  TokenInfo, Transfer, TransferEvent, TransferInfo, TransferableLog,
};
...
impl<'a, 'db, 'tx> Drc20Updater<'a, 'db, 'tx> {
  ...
  fn remove_inscribe_transfer_inscription(
    ...
    self.drc20_inscribe_transfer.remove(&value)?;
    Ok(())
  }
  ...
  pub fn get_inscription_number_by_id(&self, inscription_id: InscriptionId) -> Result<u64> {
